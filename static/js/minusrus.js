...
!(function (d) {
			...
			x === ('' + p).slice(0, I) && p.slice(I).split(':')[0] in k
				? ((w = n()),
				  ...
				  !h &&
						((h = !0),
						k[(g = b)] ||
							((h = !1), R(w.type + ' No settings for ' + g + '. Message was: ' + p)),
						h) &&
						(O(b, 'Received: ' + p),
						(g = !0),
						null === w.iframe && (R(b, 'IFrame (' + w.id + ') not found'), (g = !1)),
						g && o() && m()))
				: E(b, 'Ignored: ' + p)
	}
	function T(e, n, i) {
	...
	function C(e) {
		null !== M &&
			(window.scrollTo(M.x, M.y), O(e, 'Set page position: ' + M.x + ',' + M.y), S())
	}
	function S() {
		M = null
	}
...
function init() {
	const widgets = document.querySelectorAll('ins[data-wrapper="minusrus-widget"]')
	widgets.forEach((widget) => {
		const minusrusWidget = {
			endpointDomain: 'https://russialoses-dev.herokuapp.com/',
			endpointURI: 'widgets/stats',
			config: {
				lang: widget.dataset.lang ? widget.dataset.lang : 'ua'
			},
			...
			getEndpoint: function () {
				return `${this.endpointDomain}${this.getLangPath()}${this.endpointURI}`
			},
			createIframe: function () {
				const iframe = document.createElement('iframe')
				iframe.src = this.getEndpoint()
              ...
              `
				...
				iframe.title =
					this.config.lang === 'ua'
						? 'russianWarship.RIP - загальні бойові втрати російського окупанта'
						: 'russianWarship.RIP - the total losses of the russian occupier'
				return iframe
			},
			getWidth: function () {
				const width = widget.dataset.width
				return !isNaN(width) && !isNaN(parseFloat(width))
					? `${parseFloat(width)}px`
					: '500px'
			},
			iframeInit: function (widget) {
				const placeForIframe = widget
				if (!placeForIframe) {
					console.log(`Cant find element with data-wrapper=${this.config.widgetWrapper}`)
					return
				}
				placeForIframe.style.cssText = `
              ...
              `
				...
				iframe.addEventListener('load', () => {
					...
					placeForIframe.style.opacity = 1
				})
			}
		}
		minusrusWidget.iframeInit(widget)
	})
}
